		...
		<table class="table table-striped table-hover">
			...
			<?php foreach ($this->rests as $rest): ?>
				<tr class="rest" data-search="<?php echo remove_accents($rest['name'].' '.$rest['address'].' '.$rest['postcode'].' '.$rest['city'].' '.$rest['tel']); ?>">
					...
					<td><a class="d-block" href="https://www.google.com/maps/dir/?api=1&origin=&destination=<?php echo urlencode($rest['address'].', '.$rest['postcode'].' '.$rest['city']); ?>"><?php echo $rest['address'].', '.$rest['postcode'].' '.$rest['city']; ?></a><?php if (!empty($rest['note'])) echo ' - '.$rest['note']; ?></td>
